...
NORMAL_STYLE = Style.from_dict({"head": "fg:green", "message": "fg:silver",})

ERROR_STYLE = Style.from_dict({"head": "fg:red", "message": "fg:white",})

DEBUG_PROMPT_STYLE = Style.from_dict({"prompt": "blue",})


def print_output(head: str, message: str, style=NORMAL_STYLE) -> None:
    """Print prompt-toolkit tokens to output."""
    tokens = FormattedText(
        [("class:head", "{0} ".format(head)), ("class:message", message), ("", ""),]
    )
